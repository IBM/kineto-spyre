...
class RunProfileData:
    ...
    def _preprocess_file(trace_path, cache_dir):
        ...
        trace_json, json_reencode = RunProfileData._run_acelyzer(trace_path, trace_json, json_reencode)
    ...
    @staticmethod
    def _run_acelyzer(trace_path: str, trace_json: Dict, json_reencode: bool):
        acelyzer_opt = os.environ.get('ACELYZER_OPT', '')
        is_aiu = trace_json.get('deviceProperties', [{}])[0].get('type') == 'AIU'
        if acelyzer_opt != 'disable' and is_aiu:
            try:
                ...
                acelyzer_profile_path = os.path.join(os.path.dirname(__file__),
                                                     "../acelyzer/profiles/torch_minimal.json")
